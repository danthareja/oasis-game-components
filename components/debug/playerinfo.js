...
export class PlayerInfo extends React.Component {
  ...
  render() {
    let players = [];
    for (let i = 0; i < this.props.ctx.num_players; i++) {
      const playerID = i + 1;  // Player ids are integers and start at 1.

      let className = 'player';

      if (playerID === this.props.ctx.current_player) {
        className += ' current';
      }

      if (playerID === this.props.player_id) {
        className += ' active';
      }
